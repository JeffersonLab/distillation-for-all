...
def output_chroma_shell_job(chroma_args, geom, output_files, nodes, maxtime, job_name, output, facility_name, filename):
    ...
    if len(facility) == 0:
        raise Exception('The facility `{}` does not appear in `facilities.json`'.format(facility_name))
    facility = facility[0]
...
def process_args():
    ...
    parser.add_argument("--output-files", required=False, nargs='+', help="Files to be removed before the execution", default=[])
    parser.add_argument("--chroma-arguments", required=True, nargs='+', help="Launch a chroma job with these arguments", default=[])
    parser.add_argument("--geom", required=True, nargs=4, help="Number of processes in each lattice direction")
    parser.add_argument("--facility", required=True, nargs=1, help="This facility name")
    parser.add_argument("--job-output", required=True, nargs=1, help="Full path of the job's output")
    parser.add_argument("--nodes", required=True, nargs=1, type=int, help="Number of nodes running chroma")
    ...
    parser.add_argument("--output-shell-file", nargs=1, required=True, help="filename of the created the shell script")
    args = parser.parse_args()
    output_chroma_shell_jobs(chroma_args=args.chroma_arguments, geom=args.geom, remove_files=args.output_files, nodes=args.nodes, maxtime=args.maxtime, job_name=args.job_name, output=args.job_output, facility_name=args.facitlity, filename=args.output_shell_file)
